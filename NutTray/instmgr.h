...
private:
   ...
   void Write();
   InstanceConfig ReadConfig(HKEY key, const string & id);
   list<InstanceConfig>::iterator FindInstance(const string & id);
   string CreateId();

   bool RegQueryDWORD(HKEY hkey, const char* name, DWORD &result);
   bool RegQueryString(HKEY key, const char* name, string &result);
   void RegSetDWORD(HKEY key, const char* name, DWORD value);
   void RegSetString(HKEY key, const char* name, const string &value);

   static const TCHAR *INSTANCES_KEY;
   static const MonitorConfig DEFAULT_CONFIG;

   list<InstanceConfig> _instances;
   HINSTANCE _appinst;
   BalloonMgr _balmgr;
};
