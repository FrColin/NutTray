...
// Implementation of a system tray icon & menu for winut
     ...
     _configdlg(appinst, instmgr)
{
   ...
   _tbcreated_msg = RegisterWindowMessage(TEXT("TaskbarCreated"));
   ...
   wndclass.lpszClassName = NUTTRAY_WINDOW_CLASS;
   wndclass.hIconSm = LoadIcon(NULL, IDI_APPLICATION);
   RegisterClassEx(&wndclass);

   // Make unique window title as 'host:port'.
   TCHAR title[1024];
   swprintf(title, sizeof(title), TEXT("%s:%d"), wstring(mcfg.host.begin(),mcfg.host.end()).c_str(), mcfg.port);

   // Create System Tray menu window
   _hwnd = CreateWindow(NUTTRAY_WINDOW_CLASS, title, WS_OVERLAPPEDWINDOW,
...
upsMenu::~upsMenu()
{
   ...
   UnregisterClass(NUTTRAY_WINDOW_CLASS, _appinst);
}
...
void upsMenu::SendTrayMsg(DWORD msg)
{
   ...
   nid.uCallbackMessage = WM_NUTTRAY_NOTIFY;
   ...
   nid.uFlags |= NIF_TIP;
   if (_upsname == TEXT("UPS_IDEN") || _upsname == TEXT("<unknown>"))
	   swprintf(nid.szTip, sizeof(nid.szTip), TEXT("%ls"), statstr.c_str());
   else
	   swprintf(nid.szTip, sizeof(nid.szTip), TEXT("%ls: %ls"),
                _upsname.c_str(), statstr.c_str());
...
LRESULT upsMenu::WndProcess(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_COMMAND:
      ...
      case IDM_REMOVE:
         ...
         PostMessage(hwnd, WM_NUTTRAY_REMOVE, 0, (LPARAM)(_config.id.c_str()));
         break;

      case IDM_REMOVEALL:
         // User wants to remove all nuttray instances from registry
         PostMessage(hwnd, WM_NUTTRAY_REMOVEALL, 0, 0);
         break;

      case IDM_ADD:
         // User selected Add from the tray menu
         PostMessage(hwnd, WM_NUTTRAY_ADD, 0, 0);
         break;
      ...
      case IDM_AUTOSTART:
      {
         ...
         _instmgr->SetAutoStart((mii.fState & MFS_CHECKED)!=0);
         break;
      }
   ...
   case WM_NUTTRAY_NOTIFY:
      ...
      if (lParam == WM_RBUTTONUP)
      {
         ...
         // Set UPS name field
         ModifyMenu(_hsubmenu, IDM_NAME, MF_BYCOMMAND|MF_STRING, IDM_NAME,(wstring(TEXT("UPS: ")) + _upsname).c_str());

         // Set HOST field
         char buf[100];
         snprintf(buf, sizeof(buf), "HOST: %s:%d", _config.host.c_str(), _config.port);
         ModifyMenuA(_hsubmenu, IDM_HOST, MF_BYCOMMAND|MF_STRING, IDM_HOST, buf);

         // Set autostart field
         MENUITEMINFO mii;
         mii.cbSize = sizeof(MENUITEMINFO);
         mii.fMask = MIIM_STATE;
         mii.fState = MFS_ENABLED | (_instmgr->IsAutoStart() ? MFS_CHECKED : MFS_UNCHECKED);
         SetMenuItemInfo(_hsubmenu, IDM_AUTOSTART, FALSE, &mii);
   ...
   // The user wants nuttray to quit cleanly...
   case WM_CLOSE:
      PostQuitMessage(0);
      return 0;
   ...
   default:
      if (iMsg == _tbcreated_msg)
      {
         ...
         PostMessage(hwnd, WM_NUTTRAY_RESET, _generation++, 0);
      }
      break;
   }
...
DWORD WINAPI upsMenu::StatusPollThread(LPVOID param)
{
   ...
   while (_this->_runthread)
   {
      ...
      WaitForSingleObject(_this->_mutex, INFINITE);
      if (_this->_reconfig)
      {
         // Recreate statmgr with new config
         delete _this->_statmgr;
         _this->_statmgr = new StatMgr(_this->_config.host.c_str(), _this->_config.port);
         _this->_reconfig = false;
      }
	  ReleaseMutex(_this->_mutex);
