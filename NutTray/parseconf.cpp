...
static void add_arg_word(PCONF_CTX *ctx)
{
	...
	if (ctx->numargs > ctx->maxargs) {
		...
		/* resize the lists */
		ctx->arglist = (char**) realloc(ctx->arglist, sizeof(char *) * ctx->numargs);

		if (!ctx->arglist)
			pconf_fatal(ctx, "realloc arglist failed");

		ctx->argsize = (size_t*) realloc(ctx->argsize, sizeof(int *) * ctx->numargs);
	...
	strncpy_s(ctx->arglist[argpos], ctx->argsize[argpos], ctx->wordbuf, wbuflen);
}
...
int pconf_init(PCONF_CTX *ctx, void errhandler(const char *))
{
	...
	ctx->wordbufsize = 16;
	ctx->wordbuf = (char*) malloc(ctx->wordbufsize);
