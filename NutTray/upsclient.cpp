...
const char *upscli_strerror(UPSCONN *ups)
{
	...
	switch (upscli_errlist[ups->upserror].flags) {
		...
		{
			char errormsg[4096];
			strerror_s(errormsg, sizeof(errormsg), ups->syserrno);
		...
		case 3:		/* parsing (parseconf) error */
			snprintf(ups->errbuf, UPSCLI_ERRBUF_LEN,
				upscli_errlist[ups->upserror].str, "TODO ....33");//	ups->pc_ctx->errmsg);
			return ups->errbuf;
	}
...
int upscli_connect(UPSCONN *ups, const char *host, int port, int flags)
{
	...
	ups->pc_ctx = (PCONF_CTX*)malloc(sizeof(PCONF_CTX));
...
int upscli_list_start(UPSCONN *ups, unsigned int numq, const char **query)
{
	...
	if ((_stricmp(ups->pc_ctx->arglist[0], "BEGIN") != 0) || 
		(_stricmp(ups->pc_ctx->arglist[1], "LIST") != 0)) {
		ups->upserror = UPSCLI_ERR_PROTOCOL;
		return -1;
	}
			...
			int *port)
{
	...
	if (!ap) {
		printf( "upscli_splitname: no UPS name specified (upsname@hostname)\n");
		return -1;
	}
	...
	*upsname = _strdup(tmp);
	...
	if (!*upsname) {
		printf( "upscli_splitname: strdup failed\n");
		return -1;
	}
