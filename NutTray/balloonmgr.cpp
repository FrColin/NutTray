...
void BalloonMgr::PostBalloon(HWND hwnd, const wstring &title, const wstring &text)
{
   lock();
...
void BalloonMgr::post()
{
   ...
   nid.uID = IDI_NUTUPSD;
   nid.uFlags = NIF_INFO;
   lstrcpyn(nid.szInfo, balloon.text.c_str(), sizeof(nid.szInfo));
   lstrcpyn(nid.szInfoTitle, balloon.title.c_str(), sizeof(nid.szInfoTitle));
